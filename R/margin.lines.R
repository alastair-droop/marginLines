margin.lines <- function(from, to, from.line=1, to.line=1, side=1, ...){
	...
	old.xpd <- par("xpd")
	par(xpd=NA)
	if(side %in% c(1, 3)) segments(x0=from, y0=from.line.coordinates, x1=to, y1=to.line.coordinates, ...)
	else segments(x0=from.line.coordinates, y0=from, x1=to.line.coordinates, y1=to, ...)
	par(xpd=old.xpd)
}
